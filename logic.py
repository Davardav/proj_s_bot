...
values(?, ?, ?, ?)"""
    ...
    def get_statuses(self):
        sql = sql="SELECT status_name from status"
...
WHERE user_id = ? AND project_id = ? """
    ...
    def delete_skill(self, project_id, skill_id):
        sql = sql = """DELETE FROM skills 
WHERE skill_id = ? AND project_id = ? """
        self.__executemany(sql, [(skill_id, project_id)])
    

    def delete_status(self, status_id):
        sql =  sql = """DELETE FROM status 
WHERE status_id = ? """
        self.__executemany(sql, [status_id])

    def get_status(self, status_id):
        sql = sql="""SELECT * FROM status 
WHERE status_id = ?"""
        return self.__select_data(sql, data = (status_id,))
